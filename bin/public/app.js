...
Main.__name__ = true;
Main.main = function() {
	var r = new tink_web_proxy_Remote14(new tink_http_clients_JsClient(),tink_web_proxy__$Remote_RemoteEndpoint_$Impl_$._new(tink_url__$Host_Host_$Impl_$._new("localhost",3000)));
	var uploadform = window.document.forms.namedItem("upload_form");
	uploadform.addEventListener("submit",function(e) {
		...
		reader.onload = function() {
			...
			var this1 = tink_http__$StructuredBody_UploadedFile_$Impl_$.ofBlob(file.name,file.type,arrayBuffer);
			var tmp = this1;
		};
		reader.readAsArrayBuffer(file);
		e.preventDefault();
	});
	r.getFilesList().handle(function(o) {
		switch(o[1]) {
		case 0:
			var result = o[2];
			console.log(result);
	...
	,jsRequest: function(req) {
		...
		return tink_core__$Future_Future_$Impl_$.async(function(cb) {
			...
			http.onreadystatechange = function() {
				if(http.readyState == 4) {
					if(http.status != 0) {
						...
						var f = cb;
						var a1 = tink_core_Outcome.Failure(tink_core_TypedError.withData(502,"XMLHttpRequest Error",{ request : req, error : "Status code is zero"},{ fileName : "JsClient.hx", lineNumber : 58, className : "tink.http.clients.JsClient", methodName : "jsRequest"}));
			...
			http.onerror = function(e) {
				var tmp2 = tink_core_Outcome.Failure(tink_core_TypedError.withData(502,"XMLHttpRequest Error",{ request : req, error : e},{ fileName : "JsClient.hx", lineNumber : 64, className : "tink.http.clients.JsClient", methodName : "jsRequest"}));
				cb(tmp2);
			};
...
var tink_web_proxy_Remote14 = function(client,endpoint) {
	tink_web_proxy_RemoteBase.call(this,client,endpoint);
};
tink_web_proxy_Remote14.__name__ = true;
tink_web_proxy_Remote14.__super__ = tink_web_proxy_RemoteBase;
tink_web_proxy_Remote14.prototype = $extend(tink_web_proxy_RemoteBase.prototype,{
	uploadFile: function(body) {
		var __body__ = new tink_json_Writer7().write(body);
		var this1 = "files";
		return tink_web_proxy__$Remote_RemoteEndpoint_$Impl_$.request(tink_web_proxy__$Remote_RemoteEndpoint_$Impl_$.sub(tink_web_proxy__$Remote_RemoteEndpoint_$Impl_$.sub(this.endpoint,{ path : [this1], query : []}),{ headers : [new tink_http_HeaderField(tink_http__$Header_HeaderName_$Impl_$.ofString("content-type"),"application/json"),new tink_http_HeaderField(tink_http__$Header_HeaderName_$Impl_$.ofString("content-length"),tink_http__$Header_HeaderValue_$Impl_$.ofInt(__body__.length))]}),this.client,"POST",tink_io__$Source_Source_$Impl_$.ofChunk(tink__$Chunk_Chunk_$Impl_$.ofBytes(haxe_io_Bytes.ofString(__body__))),function(header,body1) {
			return tink_core__$Promise_Promise_$Impl_$.ofOutcome(tink_core_Outcome.Success(new tink_http_IncomingResponse(header,body1)));
		});
	}
	,getFilesList: function() {
		...
		var this1 = "files";
		var this11 = "list";
		return tink_web_proxy__$Remote_RemoteEndpoint_$Impl_$.request(tink_web_proxy__$Remote_RemoteEndpoint_$Impl_$.sub(this.endpoint,{ path : [this1,this11], query : []}),this.client,"GET",tink_io__$Source_Source_$Impl_$.ofChunk(tink__$Chunk_Chunk_$Impl_$.ofBytes(haxe_io_Bytes.ofString(__body__))),tink_web_proxy__$Remote_ResponseReader_$Impl_$.ofStringReader(($_=new tink_json_Parser0(),$bind($_,$_.tryParse))));
	}
	,__class__: tink_web_proxy_Remote14
});
